...
Ray Camera::Raycast(float x, float y)
{
    ...
    //xs and xy are NDC
    float xs = (2*x/width)-1;
    float ys = 1-(2*y/height);

    glm::vec4 viewPoint = glm::vec4(xs,ys,1,1);
    glm::vec4 P = glm::inverse(getViewProj())*viewPoint*far_clip;
    glm::vec3 rayOrigin = eye;
    glm::vec4 rayDirectionFour = glm::normalize(P-glm::vec4(rayOrigin[0],rayOrigin[1],rayOrigin[2],1));
    glm::vec3 rayDirection = glm::vec3(rayDirectionFour[0],rayDirectionFour[1],rayDirectionFour[2]);

    return Ray(glm::vec3(rayOrigin), glm::vec3(rayDirection));
}
...
Ray Camera::RaycastNDC(float ndc_x, float ndc_y)
{
    ...
    // and returns a ray generated from them.
    glm::vec4 viewPoint = glm::vec4(ndc_x,ndc_y,1,1);
    glm::vec4 P = glm::inverse(getViewProj())*viewPoint*far_clip;
    glm::vec3 rayOrigin = eye;
    glm::vec4 rayDirectionFour = glm::normalize(P-glm::vec4(rayOrigin[0],rayOrigin[1],rayOrigin[2],1));
    glm::vec3 rayDirection = glm::vec3(rayDirectionFour[0],rayDirectionFour[1],rayDirectionFour[2]);
